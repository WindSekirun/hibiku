...
- **주요 UI 섹션**:
  ...
  - **M3 앨범아트 셰이프 선택기**: Material 3 공식 셰이프 시스템(Circle, Squircle, 12각 Scallop 별꽃, Clover 플라워, Cut Corner 옥타곤, Arch 아치, Pill 타원) 중 선택.
  - **링 스타일 선택기**: 5가지 스타일(물결 웨이브, 플로팅 듀얼, 세그먼트, 글로우 썸, 솔리드 클래식)을 시각적 카드로 제공.
  - **색상 커스텀**:
...
- **반응형 UI 레이아웃 (Compose 기반)**:
  ...
  - **앨범아트 메인 컬러 자동 추출 (Palette API)**:
    - `androidx.palette:palette-ktx`를 통해 앨범아트 비트맵에서 Dominant / Vibrant 컬러를 추출.
    - 추출된 메인 컬러를 대형 Pill Play 버튼, Squiggly 물결 탐색바, 배경 앰비언트 방사형 블러 글로우에 동적 반영하여 곡 분위기와 완벽히 어우러지는 일체감 선사.
  - **배경**: 올레드 친화적 딥 블랙 (#0b0c0e) + 앨범아트 추출 색상 기반의 은은한 앰비언트 방사형 블러 글로우.
